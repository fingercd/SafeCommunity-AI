...
def main() -> None:
    ...
    ap.add_argument("--batch_size", type=int, default=16, help="4090 可用 32～64，显存小可改 8 或 16")
    ...
    ap.add_argument(
        "--save_format",
        type=str,
        default="npz_per_video",
        choices=["npy_per_clip", "npz_per_video"],
        help="缓存格式：每视频一个 .npz（推荐，训练 I/O 快）或每 clip 一个 .npy",
    )
    ap.add_argument(
        "--skip_existing",
        action="store_true",
    ...
    ap.add_argument(
        ...
        action="store_true",
        default=False,
        help="启用 SSIM 过滤（默认关闭，与 MIL 32-clips 无过滤设计一致；使用 YAML 时以 YAML 为准）",
    )
    ap.add_argument(
        "--enable_flow_filter",
        action="store_true",
        default=False,
        help="启用光流幅值过滤（默认关闭；使用 YAML 时以 YAML 为准）",
    )
