...
- 每视频一次性加载所有所需帧（去重后一次 read），减少 I/O。
- backend=farneback 时使用多线程（--workers）并行处理多视频；backend=raft 时单线程单 GPU。
"""
...
def _collect_all_clip_indices(
    ...
    """
    收集单个视频所有 clip 的帧索引（去重、排序），并返回每个 clip 的 (ci, indices)。
    返回: (all_indices, [(ci, indices), ...])
    """
    all_indices_set: set[int] = set()
    clip_index_lists: list[tuple[int, list[int]]] = []
    for ci in range(num_clips):
        indices_local, _, _ = uniform_clip_indices(
            frame_count=span,
            clip_len=clip_len,
        ...
        clip_index_lists.append((ci, indices))
    all_indices = sorted(all_indices_set)
    return all_indices, clip_index_lists
...
def process_one_video(
    ...
    resize_tuple: Optional[tuple[int, int]],
) -> tuple[Optional[dict[str, Any]], Optional[Path], Optional[str]]:
    """
    处理单个视频（仅 Farneback）：一次性加载所有所需帧，按 clip 组装并算光流，写 npz。
    返回 (meta_line, flows_npz_path, error)。成功时 error 为 None；失败时 meta_line 与 path 可为 None。
    """
    video_path = row.video_path
    abs_path = dataset_root / video_path if not Path(video_path).is_absolute() else Path(video_path)
    if not abs_path.exists():
        return None, None, f"video not found: {abs_path}"

    vid_safe = safe_id(row.video_id)
    video_out_dir = out_dir / vid_safe
    flows_npz = video_out_dir / "flows.npz"
    if skip_existing and flows_npz.exists():
        return (
            {
    ...
    try:
        ...
        return None, None, f"cannot open {abs_path}: {e}"
    ...
    all_indices, clip_index_lists = _collect_all_clip_indices(
        span, clip_len, frame_stride, num_clips_per_video, range_start
    )
    if not all_indices:
        all_flows = []
        clip_indices_list = []
        for ci, _ in clip_index_lists:
            h, w = 224, 224
            if resize_tuple:
                w, h = resize_tuple[0], resize_tuple[1]
            all_flows.append(np.zeros((max(0, clip_len - 1), 2, h, w), dtype=np.float32))
            clip_indices_list.append(ci)
    else:
        frames = read_frames_by_indices_cv2(abs_path, all_indices, to_rgb=True)
        frames_dict = dict(zip(all_indices, frames))
        all_flows = []
        clip_indices_list = []
        default_h, default_w = 224, 224
        if resize_tuple:
            default_w, default_h = resize_tuple[0], resize_tuple[1]
        for ci, indices in clip_index_lists:
            clip_frames = [frames_dict[idx] for idx in indices]
            if len(clip_frames) < 2:
                all_flows.append(
                    np.zeros((max(0, clip_len - 1), 2, default_h, default_w), dtype=np.float32)
                )
                clip_indices_list.append(ci)
                continue
    ...
    return meta_line, flows_npz, None
...
def main() -> None:
    ...
    ap.add_argument("--num_clips_per_video", type=int, default=16)
    ap.add_argument(
        "--backend",
        type=str,
        default="raft",
    ...
    ap.add_argument(
        ...
        type=int,
        default=32,
        help="Farneback 时并行处理的视频数（线程数）；raft 时忽略，单线程",
    )
    args = ap.parse_args()
        ...
        with ThreadPoolExecutor(max_workers=int(args.workers)) as executor:
            future_to_vi = {
                executor.submit(
                    ...
                    args.clip_len,
                    args.frame_stride,
                    args.num_clips_per_video,
                    resize_tuple,
                ): vi
                for vi, row in enumerate(rows)
            }
            results: list[tuple[int, Optional[dict[str, Any]]]] = []
            for future in as_completed(future_to_vi):
                vi = future_to_vi[future]
                try:
                    meta_line, path, error = future.result()
                    if error:
                        print(f"  [{vi + 1}/{total}] error: {error}")
                    else:
                        if meta_line is not None:
                            results.append((vi, meta_line))
                            vid = meta_line.get("video_id", "")
                            print(f"  [{vi + 1}/{total}] done: {vid}")
                except Exception as e:
                    print(f"  [{vi + 1}/{total}] exception: {e}")
            results.sort(key=lambda x: x[0])
            meta_lines = [m for _, m in results if m is not None]

    else:
        # RAFT: 单线程，每视频一次性加载帧后按 clip 算光流
        for vi, row in enumerate(rows):
            video_path = row.video_path
            abs_path = dataset_root / video_path if not Path(video_path).is_absolute() else Path(video_path)
            if not abs_path.exists():
