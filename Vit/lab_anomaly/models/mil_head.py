...
可选异常分数分支（借鉴 CVPR 2018 MIL Ranking Loss 论文）：
  - 对每个 clip 输出标量异常分数（0~1），供 MIL 排序损失训练
  - 结构：768 -> 512 -> 32 -> 1 + Sigmoid
"""
...
class MILHeadConfig:
    ...
    # --- 异常分数分支（借鉴 CVPR 2018 MIL Ranking Loss） ---
    use_anomaly_branch: bool = False
    anomaly_hidden: int = 512  # 论文结构：D -> 512 -> 32 -> 1
...
class MILClassifier(nn.Module):
    ...
    def __init__(self, cfg: MILHeadConfig):
        ...
        # --- 异常分数分支（per-clip 标量异常分数，用于 MIL 排序损失） ---
        self.use_anomaly_branch = bool(getattr(cfg, "use_anomaly_branch", False))
        if self.use_anomaly_branch:
            ah = int(getattr(cfg, "anomaly_hidden", 512))
            self.anomaly_scorer = nn.Sequential(
                nn.Linear(cfg.embedding_dim, ah),
    ...
    def forward(
        ...
        # --- 异常分数分支：对每个 clip 输出标量异常分数 (B, N) ---
        if self.use_anomaly_branch:
            anomaly_scores = self.anomaly_scorer(x).squeeze(-1)  # (B, N)
