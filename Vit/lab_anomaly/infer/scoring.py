...
推理阶段打分模块：
  ...
  - fuse_known_and_open_set：已知分类概率 + 开放集 anomaly + MIL ranking 异常分数的三路融合
"""
...
@dataclass(frozen=True)
class FusionOutput:
    """融合输出：预测标签、置信度、anomaly_score、是否异常、开放集信息、ranking 异常分数"""
    predicted_label: str
    ...
    ranking_anomaly_score: float = 0.0  # MIL ranking 异常分数（0~1，来自异常分支 Sigmoid 输出）
...
def fuse_known_and_open_set(
    ...
    """
    已知分类 + 开放集 + MIL ranking 异常分数的三路融合。

    - 已知分类：取 argmax 作为预测；低置信度视为 unknown
    - 开放集 anomaly_score 与 is_anomaly
    - ranking_anomaly_score：来自 MIL 异常分支的 clip 级异常分数（取 max）
    - 三路中任一触发均视为异常
    """
    ...
    # 三路异常判断：开放集 OR ranking 分支
    is_anomaly = bool(open_set.is_anomaly) or (float(ranking_anomaly_score) >= float(ranking_alarm_threshold))
