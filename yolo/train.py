...
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"  # 避免显存碎片干扰
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = r'C:\Users\Administrator\Desktop\yolov8-pytorch-master\logs\best_epoch_weights.pth'
    #------------------------------------------------------#
    ...
    phi             = 'l'
    #----------------------------------------------------------------------------------------------------------------------------#
    #   pretrained      是否使用主干网络的预训练权重，此处使用的是主干的权重，因此是在模型构建的时候进行加载的。#woker
    ...
    #                   评估需要消耗较多的时间，频繁评估会导致训练非常慢    #   此处获得的mAP会与get_map.py获得的会有所不同，原因有二：
    #   （一）此处获得的mAP为验证集的mAP。
    ...
    #------------------------------------------------------#
    train_annotation_path = r"C:\Users\Administrator\Desktop\yolov8-pytorch-master\Datasets\Labels\train.txt"  # 合并后的训练集TXT
    val_annotation_path = r"C:\Users\Administrator\Desktop\yolov8-pytorch-master\Datasets\Labels\val.txt"  # 合并后的验证集TXT
    ...
    # 添加在 num_val = len(val_lines) 之后
    print(f"[诊断] 训练集样本数: {num_train}")
    print(f"[诊断] 验证集样本数: {num_val}")
    print(f"[诊断] 验证集文件路径: '{val_annotation_path}'")
    print(f"[诊断] 验证集前3行内容: {val_lines[:3] if len(val_lines) >= 3 else val_lines}")
    if local_rank == 0:
        show_config(
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                gen.batch_sampler.batch_size = Unfreeze_batch_size
                gen_val.batch_sampler.batch_size = Unfreeze_batch_size
